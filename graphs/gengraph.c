...
/*
  ...
  CHECK>0, alors le graphe sera stockÃ© en mÃ©moire. Si
  CHECK>1=CHECK_ON, alors en plus l'algorithme spÃ©cifique sera
  appliquÃ© au graphe de sortie.
*/
...
/*
  ...
  forme compacte. N'affiche rien si le graphe est vide ou si
  G=NULL. Utilise WIDTH. Effet de bord: le (ou les) graphes sont triÃ©s
  par ordre croissant, et donc G->sort=1 en sortie. Si le graphe est
  asymÃ©trique, des sommets peuvent Ãªtre affichÃ©s comme sommets isolÃ©s
...
*/
{
  ...
  for(k=0;k<nk;k++){
    ...
    if(H==NULL || H->n<=0) continue; // ne rien faire si le graphe est vide
...
/*
  ...
  code=0). Renvoie NULL si le graphe rÃ©sultant est vide. Mettre T=NULL
  pour une liste vide.

  On peut ainsi faire une copie C du graphe G simplement en faisant
  (voir aussi la fonction GraphCopy):
...
/*
  GÃ©nÃ¨re, Ã  partir d'une permutation P de [0,n[, la prochaine
  permutation dans l'ordre lexicographique suivant les contraintes
  (intervalles de positions) dÃ©finies par le tableau C. On permute les
  Ã©lÃ©ments de P que si leurs positions sont dans l'intervalle
  [C[t-1],C[t][ pour un certain indice t, en posant C[-1]=0 par
  convention. Mettre C=NULL s'il n'y a pas de contrainte
  particuliÃ¨re. On renvoie 1 si la prochaine permutation a pu Ãªtre
  dÃ©terminÃ©e et 0 sinon, c'est-Ã -dire si P Ã©tait la derniÃ¨re
  permutation. Dans ce dernier cas la permutation la plus petite selon
  l'ordre lexicographique est renvoyÃ©e, soit P[i]=i.
 
  La fonction continue de fonctionner si C=NULL et si les valeurs de P
  ne sont pas strictement dans [0,n[, Ã  savoir construire la prochaine
  permutation selon l'ordre lexicographique. Cela ne marche pas si
  C<>NULL (voir l'exemple ci-aprÃ¨s). Cependant, lorsque la derniÃ¨re
  permutation est atteinte, la plus petite devient P[i]=i.
  ...
  plus petite P[i]=i. Sinon le rÃ©sultat peut ne plus Ãªtre une
...
int *Prune(const graph* G,int *z,int code)
/*
  Renvoie un ordre d'Ã©limination des sommets par degrÃ© minimum. Le
  rÃ©sultat est un tableau T de taille G->n de sorte que u=T[i] est un
  sommet de degrÃ© minimum du graphe G_i dÃ©fini par:

           G_i := G \ { T[0] âˆª ... âˆª T[i-1] }

  Le graphe G n'est pas modifiÃ©. La complexitÃ© est linÃ©aire,
  i.e. O(n+m). Si *z<>NULL, on retourne dans z une valeur dÃ©pendante
  de code, comme ceci:

  - Si code=0, alors en retour, z vaut le degrÃ© maximum rencontrÃ©,
    soit la dÃ©gÃ©nÃ©rescence du graphe, ou encore la valeur max_i
    deg_{G_i}(T[i]).

  - Si code=1, alors en entrÃ©e z vaut un seuil si bien que seuls les
    sommets u de degrÃ© <= z sont placÃ©s dans T, et le processus
    s'arrÃªte. Au retour z vaut le nombre de sommets placÃ©s
    correctement dans T. Si *z=NULL, alors on ne fait rien et on
    renvoie NULL. On peut ainsi facilement supprimer tous les sommets
    isolÃ©s d'un graphe, voir CHECK_PRUNE.
*/
{
  int i,j,u,v,d,k,p,r,t,c;
  int const n=G->n;
  int const n1=n-1;
  ...
  /*
    ...
    T[0..n[ = tableau de sommets supprimÃ©s par degrÃ© minimum (le rÃ©sultat)
    R[0..n[ = tableau de positions dans T des sommets (T[i]=u <=> R[u]=i)
    D[0..n[ = tableau des degrÃ©s des sommets (D[u]=deg dans G_i)
    P[0..n[ = tableau des positions dans T du premier sommet de degrÃ© donnÃ©
    
    2. On traite les sommets dans l'ordre T[0], T[1], ... Supposons
    que u = T[i]. Alors, pour chaque voisin v dans G, encore existant
    (donc situÃ© aprÃ¨s i dans le tableau T), on met Ã  jour les tables
    T,R,D,P mais sans modifier G.
  */

  if(n<1) return NULL; // rien Ã  faire dans ce cas
  if(code==1 && z==NULL) return NULL; // rien Ã  faire

  /* initialise T,R,D,P en temps O(n) */
  NALLOCZ(int,D,n,G->d[_i]);
  int *R=SortInt(D,NULL,n,0,NULL,SORT_INDEXe);
  NALLOC(int,T,n); for(u=0;u<n;u++) T[R[u]]=u;
  NALLOCZ(int,P,n,-1);
  for(i=n1;i>=0;i--) P[D[T[i]]]=i; // i=n-1,...,0
  c=0; if(code==1) c=*z; // c=0 par dÃ©faut sauf si code=1

  /* parcourt G en temps O(n+m) */
  for(i=0;i<n1;i++){ // pour chaque sommet, pas besoin de faire le dernier
    u=T[i]; // u = sommet de degrÃ© (D[u]) minimum
    if(D[u]>c){
      if(code==0) c=D[u]; // mÃ©morise le plus grand degrÃ© rencontrÃ©
      else if(code==1) break;  // deg min trop grand, on sort
    }
    d=G->d[u]; // d = deg(u)
  ...
  if(z && code==0) *z=c; // renvoie la dÃ©gÃ©nÃ©rescence
  if(z && code==1){
    if(i==n1 && c>=0) i++; // gÃ¨re le dernier sommet 
    *z=i; // renvoie le nombre de valeurs dans T
  }
...
int biggs(query* const Q)
/*
  Utilise i<j.
  
  Les sommets sont partitionnÃ©s en six blocs de n sommets de numÃ©ro
  consÃ©cutifs. Les blocs sont connectÃ©s selon le H graphe selon un
  produit cartÃ©sien: les sommets homologues dans chaque blocs (de mÃªme
  modulo n) sont adjacents. Chaque bloc correspond Ã  un anneau de
  cordes de longueur c=0,1,2,4,8 rÃ©partis comme ceci:
  ...
  Donc deux sommets de deux blocs diffÃ©rents sont adjacents si leur
  bloc sont adjacent dans H et s'ils ont mÃªmes modulo n (sommets
  homologues). Et deux sommets dans un mÃªme bloc sont adjacents si
  leurs diffÃ©rences modulo n vaut c. Sinon, ils ne sont pas adjacents.
*/
{
  int const n=Q->param[0];
  switch(Q->code){
  ...
  case QUERY_ADJ:;
    int const x=Q->i/n; // numÃ©ro d'intervalle pour i
    int const y=Q->j/n; // numÃ©ro d'intervalle pour j
    // intervalle diffÃ©rent
    if(x<y){
      if(Q->i%n!=Q->j%n) RET_a(0); // sommets homologues ?
      if(x==4) RET_a(1); // arÃªte 4-5 de H
      if((y==5)&&(x==1||x==2)) RET_a(1); // arÃªtes 2-5 ou 1-5
      if((y==4)&&(x==0||x==3)) RET_a(1); // arÃªtes 0-4 ou 3-4
      RET_a(0);
    }
    // mÃªme intervalle
    int const c=(x<4)? 1<<x : 0; // c=0,1,2,4,8
    RET_a( (((Q->i+c)%n)==Q->j%n) || (((Q->j+c)%n)==Q->i%n) );
  }
...
int mikado(query* const Q)
/*
  Ã€ FINIR.

  mikado n: graphe planaire alÃ©atoire gÃ©nÃ©rÃ© par le processus
  suivant. On tire au hasard n segments de lignes dans le plan. Les
  sommets sont les intersections et les extrÃ©mitÃ©s des segments, et
  les arÃªtes sont les sous-segments lorsque les sommets sont supprimÃ©s
  (c'est-Ã -dire les sous-segments en deux sommets consÃ©cutifs d'un
  mÃªme segment). Tout graphe planaire sans sommet isolÃ© peut Ãªtre
  gÃ©nÃ©rÃ© par ce procÃ©der, chaque segment Ã©tant prÃ©cisÃ©ment une
  arÃªte. C'est un graphe gÃ©omÃ©trique. Pour le dessiner, il suffit de
  dessiner les segments, puis d'ajouter les intersections. On pourrait
  avoir la variante, mikado -n, oÃ¹ tous les extrÃ©mitÃ©s des segments
  (les sommets de degrÃ© 1) ont Ã©tÃ© supprimÃ©s. Le nombre maximum de
  sommets est (n+1)*(n+2)/2-1 et n*(n-1)*25/216 + 2n en moyenne (pour
  un tirage uniforme dans [0,1]^2). Dans le tirage, il ne peut avoir
  deux segments colinÃ©aires s'intersectant.

  Comme une ligne coupe au plus toutes les autres, le nombre de
  sommets maximum vÃ©rifie Nmax(n) = Nmax(n-1) + (n+1) avec Nmax(1)=2
  car un segment a deux extrÃ©mitÃ©s qui sont des sommets. Cela fait
  Nmax(n) = 2 + 3 + ... + n + (n+1) = (n+1)*(n+2)/2-1. En moyenne le
  nombre de sommets est binom(n,2)*1/3*p(4) = n(n-1)*25/216 oÃ¹
  p(4)=(5/6)^2 est la probabilitÃ© d'avoir 4 points en position
  convexe. En effet, Ã©tant donnÃ©s 4 points A,B,C,D en position
  convexe, exactement une paire de segments sur ces 4 points peut
  s'intersecter parmi les 3 cas suivants: soit AB intersecte CD, soit
  AC intersecte BD, soit AD intersecte BC. En position non-convexe
  aucune intersection n'est possible. En passant, la probabilitÃ©
  d'avoir n points du carrÃ© en position convexe vaut p(n) =
  (binom{2(n-1)}{n-1}/n!)^2 [Valtr95].
*/
{
  int const n=Q->param[0];
  ...
  case QUERY_INIT:;
    int v,i,j,i2,j2;
    int const p=2*n; // p=nombre de points, n=nombre de segments
    // tirage de 2n points qui doit Ãªtre valide: il ne faut pas deux
    // segments colinÃ©aires s'intersectant.
    SET_n(p);
    do{ 
      InitXY(Q);
      /*
	Pour tester qu'il n'y a pas deux segments colinÃ©aires et
	intersectant, disons [A,B] et [C,D] oÃ¹ A,B,C,D sont des points
	du plan, il faut faire:

	- tester si C ou D est dans [A,B] ou bien A ou B est dans [C,D] (sinon pas de problÃ¨me)
	- tester si det(B-A,C-A)=0 (sinon pas de problÃ¨me)
	- tester si det(C-A,D-A)=0 (sinon pas de problÃ¨me)
      */
      for(i=1,i2=2;i<n;i++,i2+=2){ // pour chaque paires (i,j) de segments
	point const A={Q->xpos[i2],Q->ypos[i2]};
	point const B={Q->xpos[i2+1],Q->ypos[i2+1]};
	for(j=j2=0;j<i;j++,j2+=2){
	  point const C={Q->xpos[j2],Q->ypos[j2]};
	  point const D={Q->xpos[j2+1],Q->ypos[j2+1]};
	  // voir seg_intersection();
	  v=seg_intersection(A,B,C,D);
	}
      }
    }while(v);
    SET_n(n);
    int const m=(n+1)*(n+2)/2; // nombre maximum de points
    REALLOC(Q->xpos,m);
    REALLOC(Q->ypos,m);
    return 1;
...
int main(int const argc,string argv[]){
  ...
  ******************************************************************/
  ...
  while(i<ARGC){
	...
	if EQUAL("prune"){ CHECK=CHECK_PRUNE; FORMAT=F_no; goto check0; }
    ...
    if EQUAL("mikado"){ Q->adj=mikado; POS=1; goto param1; }
    ...
    if EQUAL("biggs"){ Q->adj=biggs; goto param1; }
    //
    ...
    if EQUAL("biggs-smith"){ Q->adj=biggs; Q->param[1]=17; goto param1; }
 ...
 check:
  ...
  if(CHECK){
    ...
    case CHECK_PRUNE:{
      k=MEM(CPARAM,0,int);
      int *T=Prune(GF,&k,1); // T[0..k[ = liste des sommets Ã  supprimer
      graph *S=ExtractSubgraph(GF,T,k,0);
      PrintGraph(S); // affiche le sous-graphe
      free_graph(S);
      free(T);
    }break;
...
/*# ###
GÃ©nÃ©rateur de graphes - v5.3 - Â© Cyril Gavoille - Novembre 2020
...
DESCRIPTION
       ...
       est non orientÃ© et affichÃ© selon une liste d'arÃªtes (au format
       texte), mais d'autres formats sont possibles: liste
       d'adjacence, format dot de GraphViz, xfig ou pdf par exemple.
       En paramÃ¨tre figure le nom du graphe ainsi que ses paramÃ¨tres
       Ã©ventuels, typiquement le nombre de sommets. La commande
       appelÃ©e seule affiche l'aide sur les options du gÃ©nÃ©rateur. Si
       les paramÃ¨tres d'une option ou d'un graphe sont absents ou
       remplacÃ©s par "?", une aide spÃ©cifique est affichÃ©e. Une
       console supportant l'UTF8 est prÃ©fÃ©rable.
   ...
   LE FORMAT STANDARD
       ...
       chemins Ã©crits au format texte. Ce format minimaliste est trÃ¨s
...
....
          ...
          s'agit de la formulation multi-valuÃ©e classique, un sommet
...
....
	    ...
	    au moins une fois (il faut donc que nâ‰¥m).
....
...
....
          ...
          obtenu en supprimant rÃ©cursivement tous les sommets de
          degrÃ©s â‰¤ d.
....
...
....
          ...
          (h) appliquÃ©e le cas Ã©chÃ©ant aux sommets souvent utilisÃ©s
...
....
       Cette option permet de contrÃ´ler la sortie au format dot. Elle
...
....
          ...
          â€¢ c = crosscap = couture non-orientÃ©e
...
....
       ...
       Lorsque h=1, il s'agit d'un seul polygone, un cycle Ã  p sommets.
...
....
       ...
       complÃ©mentaire du graphe parapluie n.
...
....biggs n
....
       Graphe de Biggs-Smith gÃ©nÃ©ralisÃ©. Il est cubique sauf pour
       n=1,2,4,8,16 oÃ¹ son degrÃ© maximum est 3. Il possÃ¨de 6n sommets
       partitionnÃ©s en six blocs de n sommets de numÃ©ro consÃ©cutifs.
       Les blocs sont connectÃ©s comme le graphe H selon un produit
       cartÃ©sien: les sommets homologues dans chaque blocs (de mÃªme
       modulo n) sont adjacents. Chaque bloc (0â€¦5) induit un anneau de
       corde de longueur c=0,1,2,4,8 rÃ©partis comme ceci:
....
       !!! 2   0     [2n,3n[      [0,n[
           |   |       c=4         c=1
           |   |        |           |
	   ...
	   |   |        |           |
           1   3      [n,2n[     [3n,4n[
                       c=2         c=8
....
       Donc deux sommets de deux blocs diffÃ©rents sont adjacents si
       leurs blocs sont adjacents dans H et s'ils ont mÃªme modulo n
       (sommets homologues). Et deux sommets dans un mÃªme bloc sont
       adjacents si leurs diffÃ©rences modulo n vaut c. Sinon, ils ne
       sont pas adjacents.
...
....
       ...
       alÃ©atoirement uniformÃ©ment dans le carrÃ© [0,1[Â² (distribution
...
....
       ...
       dÃ©crit dans un paragraphe ci-aprÃ¨s. Il faut p,qâ‰¥2. Il comprend
       au plus 3n+1 faces internes rectangulaires, appelÃ©s blocs, qui
       forment une partition des cases de la grille pÃ—q. C'est un
       graphe 2-dÃ©gÃ©nÃ©rÃ© qui possÃ¨de au plus min{5n+4,pÂ·q} sommets
       dont 4 sont de degrÃ© 2, les autres Ã©tant de degrÃ© 3 ou 4 selon
       une rÃ©partition moyenne 80%-20%. La variante wpsld (ou upsld)
...
....
       ...
       ou une croix prÃ©cÃ©dente, les points se trouvant sur le passage
...
....
       ...
       alÃ©atoirement un bloc, soit proportionnellement Ã  sa surface
       ...
       une probabilitÃ© proportionnelle Ã  la longueur des cotÃ©s (wdis),
...
....
       Fixed Degree Random Graph. Graphe alÃ©atoire asymptotiquement
...
....
       ...
       les báµ¢=0 [Conway'72].
....
       La gÃ©nÃ©ration du graphe proprement dite est basÃ©e sur une file
...
....
       Odd graph de paramÃ¨tre n>0. Il s'agit du graphe de Kneser
       K(2n-1,n-1) dont les sommets correspondent aux binom(2n-1,n-1)
       sous-ensembles Ã  n-1 Ã©lÃ©ments de {0,â€¦,2n-2}. Le plus petit
       cycle impair est toujours de longueur 2n-1. Il est n-rÃ©gulier
       et son diamÃ¨tre est n-1. Il est distance-transitif
       (automorphisme entre paires de sommets Ã  mÃªme distance) et donc
       distance-rÃ©gulier. Si n>3, il est Hamiltonien et de maille
       6. Il n'est jamais un graphe de Cayley si n>2. Pour n=2, il
       s'agit de Kâ‚ƒ et pour n=3 du graphe de Petersen (et donc non
       Hamiltonien).

....biggs-smith (= biggs 17)
....
       Graphe de Biggs-Smith. Il est cubique, possÃ¨de 102 sommets, et
       est distance-rÃ©gulier. Il n'existe que 13 graphes cubiques
       distance-rÃ©guliers. De nombre chromatique 3, son diamÃ¨tre est
...
....
       ...
       avec 3 colonnes et 3 lignes parallÃ¨les joignant des sommets
